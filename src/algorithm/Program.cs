...
public class Program
{
    public static void Main(string[] args)
    {
        ...
        string pattern = "";
        // crop the patternPath image with ImageConverter.CropImageTo1x32 then conver to ascii string
        using (Image<Rgba32> patternImage = Image.Load<Rgba32>(patternPath))
        {
            using (Image<Rgba32> croppedPatternImage = ImageConverter.CropImageTo1x32(patternImage))
            {
        ...
        string result = matcher.FindMostSimilarFingerprint(pattern, referenceImagesMap, croppedReferenceImagesMap);   
